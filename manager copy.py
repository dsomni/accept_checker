...
async def save_status(attempt_spec, database):
    ...
    if result:
        await status_collection.update_one(
            {"attempt": attempt_spec}, {"$set": {"status": 2}}
        )
...
async def save_attempt_results(spec, tests, results, logs, collection):
    ...
    await collection.update_one(
        {"spec": spec},
        {"$set": {"status": 2, "verdict": verdict, "results": tests, "logs": logs}},
    )
...
async def save_task_results(
    attempt, author: str, task: str, results, verdict, verdictTest, database
):
    spec = attempt["spec"]
    ...
    new_best = None
    if best and (
        best["verdict"] == verdict == 0 or best["percentTests"] > percentTests
    ):
        new_best = best
    ...
    if (not best or best["verdict"] != 0) and new_best["verdict"] == 0:
        await database["rating"].update_one(
            {"user": author}, {"$inc": {"score": 1}}, True
        )
    ...
    if not results_db:
        ...
        await collection.update_one(
            ...
            {"$push": {"results": current_attempt, "bests": new_best}},
        )
...
async def save_results(attempt, author, task, tests, results, logs):
    ...
    (verdict, verdictTest) = await save_attempt_results(
        spec, tests, results, logs, collection
    )
    await save_task_results(
        attempt, author, task, results, verdict, verdictTest, database
    )
...
def soft_run(func):
    async def inner(attempt, author: str, task: str, *args):
        try:
            ...
            try:
                await save_results(
                    spec, author, task, tests, [5] * len(tests), [str(e)]
                )
            except:
                await send_alert(
                    "ManagerError (when saving results)", f"{spec}\n{str(e)}"
                )
            return False
...
async def tests_checker(attempt, author: str, task: str, language) -> bool:
    ...
    program_path, folder_path = create_program_file(
        FOLDER, spec, extension, attempt["programText"]
    )
...
async def text_checker(attempt, author, task) -> bool:
    ...
    for i, test_result in enumerate(tests):
        if i >= answers_length:
            ...
            results.append(
                compare_strings(test_result["test"]["outputData"], answers[i])
            )
...
async def custom_checker(attempt, author: str, task: str, language, checker) -> bool:
    ...
    if not checker_code:
        return await save_results(
            attempt, author, task, tests, [6] * len(tests), ["No checker specified"]
        )
    ...
    module_name, checker_module_spec = get_module(
        checker_lang["shortName"], langs_configs
    )
    ...
    with open(
        os.path.join(folder_path, f"{CHECKER_NAME}.{checker_extension}"), "w"
    ) as custom_checker_f:
...
async def start(*args):
    ...
    queue_item = await database["pending_task_attempt"].find_one(
    ...
    if task_type == 0:  # code
        ...
        if check_type == 0:
            ...
            checker_language = await database["language"].find_one(
